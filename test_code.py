...
from teanglann import assign_plural_genitive
from lookup_irish import run_main
# from teanglann import get_teanglann_definition, assign_plural_genitive

# adverb/preposition/adverb in a single entry
def test_run_main():
    assert run_main({'irish-word': 'anall'}) == None

# make sure to pick up the 'Adjective'
def test_run_main():
    assert run_main({'irish-word': 'buan'}) == None

# don't use variant plural breitheamhain
def test_assign_plural_genitive():
    assert assign_plural_genitive('breitheamh') == {
        'gender': 'nm1',
        'genitive plural': 'na mbreit<em>hiúna</em>',
        'genitive singular': '<i class="weak">an</i> b<i>h</i>reithimh',
        'nominative plural': 'na breit<em>hiúna</em>',
        'nominative singular': 'an breitheamh',
        'plural strength': 'strong'
        }
